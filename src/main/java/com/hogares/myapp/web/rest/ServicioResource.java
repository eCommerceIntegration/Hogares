...
/**
 * REST controller for managing {@link com.hogares.myapp.domain.Servicio}.
 */
@RestController
@RequestMapping("/api")
public class ServicioResource {
    ...
    public ResponseEntity<Servicio> updateServicio(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, servicio.getId()))
            .body(result);
    }
    ...
    public ResponseEntity<Servicio> partialUpdateServicio(
        ...
        Optional<Servicio> result = servicioRepository
            ...
            .map(
                existingServicio -> {
                    if (servicio.getServicioTitle() != null) {
                        existingServicio.setServicioTitle(servicio.getServicioTitle());
                    }
                    if (servicio.getMinSalary() != null) {
                        existingServicio.setMinSalary(servicio.getMinSalary());
                    }
                    if (servicio.getMaxSalary() != null) {
                        existingServicio.setMaxSalary(servicio.getMaxSalary());
                    }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, servicio.getId())
        );
    }
    ...
    public ResponseEntity<Servicio> getServicio(@PathVariable String id) {
