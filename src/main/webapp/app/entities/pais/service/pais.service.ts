...
export class PaisService {
  ...
  update(pais: IPais): Observable<EntityResponseType> {
    return this.http.put<IPais>(`${this.resourceUrl}/${getPaisIdentifier(pais) as string}`, pais, { observe: 'response' });
  }

  partialUpdate(pais: IPais): Observable<EntityResponseType> {
    return this.http.patch<IPais>(`${this.resourceUrl}/${getPaisIdentifier(pais) as string}`, pais, { observe: 'response' });
  }
